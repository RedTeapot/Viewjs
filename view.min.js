...
I.getDomElement().classList.add("active")};var L=function(){var R=I.getId();k(R);if(Q){if(b in w){h(R,w[b]);delete w[b]}}else{if(O){if(r in w){h(R,w[r]);delete w[r]}}else{if(f!=K.params){h(R,K.params)}}}var S={sourceView:P,type:N};if(!I.isReady()){e.push(R);I.fire("ready",S)}I.fire("beforeenter",S);I.fire("enter",S);I.fire("afterenter",S)};if(!K.withAnimation){M();L()}else{var J=function(){M();L()};if(F){F.call(null,P?P.getDomElement():null,I.getDomElement(),N,J)}else{J()}}x.fire("afterchange",{currentView:P,targetView:I,type:N})};var E=function(I,K){K=d(K,{type:x.SWITCHTYPE_VIEWSWITCH,},true);if(!x.isExisting(I)){throw new Error("Target view: "+I+" does not exist!")}var L=x.getActiveView();if(L.getId().toLowerCase()==I.toLowerCase()){return}m.log("{} → {} {}",L.getId(),I,K.type);var J=x.ofId(I);K.srcView=L;K.targetView=J;p(K);return x};x.show=E;x.switchTo=function(I,K,J,L){if(arguments.length<4){L=null}if(arguments.length<3){J=true}if(arguments.length<2){K=x.SWITCHTYPE_VIEWSWITCH}K=K||x.SWITCHTYPE_VIEWSWITCH;E(I,{type:K,withAnimation:J,params:L});return x};x.switchView=x.switchTo;x.navTo=function(I,J){I=I.trim().toLowerCase();var K=new l(I,Date.now());if(b==I){x.back(J);return}if(r==I){x.forward(J);return}E(I,J);D(I);return x};x.updateView=x.navTo;x.back=function(I){k(b);if(null!=I&&"params" in I){console.log("!!!!",w);h(b,I.params)}history.go(-1)};x.forward=function(I){k(r);if(null!=I&&"params" in I){console.log("!!!!",w);h(r,I.params)}history.go(1)};x.changeTo=function(I,J){E(I,J);t(I);return x};var o=document.title;var s=function(I){if(null==I||""==I){console.warn("Invalid document title: "+I);return}o=I};x.setDocumentTitle=s;x.Logger=G;var j;x.ready=(function(){var J=false;var I=[];j=function(){J=true;setTimeout(function(){I.forEach(function(K){K&&K()})},0)};return function(K){if(J){K&&K();return}if(I.indexOf(K)!=-1){return}I.push(K);return x}})();var B=function(I){var J=null;if(null==I||""==I){J=x.getActiveView()}else{if(!x.isExisting(I)){J=x.getActiveView()}else{if(x.ofId(I).isDirectlyAccessible()){J=x.ofId(I)}else{J=x.ofId(I).getFallbackView()}}}return J};var c=function(N){var M=x.getActiveView();m.log("{} Current: {}",C?"State poped!":"Hash changed!",M.getId());m.log("↑ {}",C?JSON.stringify(N.state):location.hash);var J,L=x.SWITCHTYPE_VIEWSWITCH,K=null;if(null==N.state){L=x.SWITCHTYPE_VIEWSWITCH;J=location.hash.replace(/^#/,"").toLowerCase();K=B(J);t(K.getId())}else{var I=N.state;J=I.viewId;if(x.isExisting(J)){K=x.ofId(J)}else{m.warn("Poped view: "+J+" does not exist, keeping current.");K=M}if(x.currentState!=null){L=I.timestamp<x.currentState.timestamp?x.SWITCHTYPE_HISTORYBACK:x.SWITCHTYPE_HISTORYFORWARD}t(K.getId(),I.timestamp)}x.show(K.getId(),{type:L})};var y=function(){window.addEventListener(C?"popstate":"hashchange",c);var K=document.querySelectorAll("*[data-view=true]");var L=null;var J=document.title;var I=function(){var N=-1;for(var M=0;M<K.length;M++){if("true"==K[M].getAttribute("data-view-default")){N=M;break}}if(-1!=N){L=K[N];for(var M=N+1;M<K.length;M++){if("true"==K[M].getAttribute("data-view-default")){K[M].removeAttribute("data-view-default")}}}else{if(0!=K.length){L=K[0];L.setAttribute("data-view-default","true")}}return L};[].forEach.call(K,function(M){x.ofId(M.id);M.classList.remove("active");I();if(null!=L){L.classList.add("active")}M.classList.add("view");(function(){var O=M.getAttribute("data-view-title");var N=x.ofId(M.id);N.on("enter",function(){document.title=null==O?J:O})})()});(function(){var P=x.getDefaultView().getId();var O=location.hash.replace(/^#/i,"").toLowerCase().trim();var M=""==O?x.getDefaultView().getId():O;var N=B(M);m.log("Initial target view: "+N.getId());var Q=l.isConstructorOf(history.state);if(null!=history.state){m.log("Found existing state: {}, isViewState? {}",JSON.stringify(history.state),Q);if(!(Q&&history.state.viewId==N.getId())){t(N.getId())}}else{t(N.getId())}p({srcView:x.getActiveView(),targetView:N,withAnimation:false})})();(function(){i.addTapListener(document.documentElement,function(Q){var P=Q.changedTouches?Q.changedTouches[0].target:Q.target;var M;var O=P;while(null==O.getAttribute("data-view-rel")){O=O.parentNode;if(!(O instanceof HTMLElement)){O=null}if(null==O){break}}if(null!=O){M=O.getAttribute("data-view-rel")}if(null==M){return}var R="true"==O.getAttribute("data-view-rel-disabled");if(R){return}Q.preventDefault();if(b==M.toLowerCase().trim()){history.go(-1);return}if(r==M.toLowerCase().trim()){history.go(1);return}var N=O.getAttribute("data-view-rel-type");N=null==N||""==N.trim()?"nav":N;if(!/^(?:nav)|(?:change)$/.test(N)){m.warn("Unknown view switch type: {}. {}",N,O);N="nav"}x[N+"To"](M,{type:x.SWITCHTYPE_VIEWSWITCH})},{useCapture:true})})();j()};document.addEventListener("DOMContentLoaded",y);v[H]=x})(window,"View");