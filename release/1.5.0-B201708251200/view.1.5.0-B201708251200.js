...
;(function(ctx, name){
	 ...
	 * 切换视图
	 * @param {String} targetViewId 目标视图ID
	 * @param {StringEnum} type 切换操作类型（View.SWITCHTYPE_HISTORYFORWARD || View.SWITCHTYPE_HISTORYBACK || View.SWITCHTYPE_VIEWNAV || View.SWITCHTYPE_VIEWCHANGE）
	 * @param {Boolean} withAnimation 是否执行动画
	 * @param {Any} params 视图参数。仅当切换操作类型为： || View.SWITCHTYPE_VIEWNAV || View.SWITCHTYPE_VIEWCHANGE时才会被使用
	 */
	...
	var init = function(){
		...
		;(function(){
			touch.addTapListener(docEle, function(e){
				...
				/* 呈现ID指定的视图 */
				View[relType + "To"](targetViewId, {options: rst.options});
			}, {useCapture: true});
		})();
